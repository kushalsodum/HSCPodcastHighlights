...
class SegmentClassificationLLM(nn.Module):
    ...
    def forward(self, input_ids, segment_end_indices):
        ...
        # Pass through LSTM
        lstm_out, _ = self.lstm(selected_hidden_states.float()) # (batch_size, 100, hidden_size)
        
        # Apply classifier to each time step
        logits = self.classifier(lstm_out) # (batch_size, 100, 1)
...
def main():
    ...
    wandb.init(entity="ksodum-university-of-michigan",
                project="final project BiLSTM testing")
    ...
    # Save model
    output_dir = "rhapsody_model_bilstm"
    os.makedirs(output_dir, exist_ok=True)
    model.base_model.save_pretrained(output_dir)
    torch.save(model.lstm.state_dict(), os.path.join(output_dir, "lstm.pt"))
